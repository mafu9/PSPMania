...
RageFileDriverZip::RageFileDriverZip( const CString &path ):
...
#define CENTRAL_HDR_SIG "\x50\x4B\x01\x02"
#define LOCAL_HDR_SIG "\x50\x4B\x03\x04"
#define END_CENTRAL_SIG "\x50\x4B\x05\x06"
...
void RageFileDriverZip::ParseZipfile()
{
	...
	if( got != 4 || strncmp(sig, CENTRAL_HDR_SIG, 4) != 0 )
    ...
    while(1)
    {
		...
		/* Is it a new entry? */
		if( strncmp(sig, CENTRAL_HDR_SIG, 4) != 0 )
		{
			...
			if( Files.size() == (unsigned)ec.total_entries_central_dir )
			{
				...
				if( strncmp(sig, END_CENTRAL_SIG, 4) != 0 )
...
RageFileObj *RageFileDriverZip::Open( const CString &path, int mode, RageFile &p, int &err )
{
	...
	if( info->data_offset == -1 )
	{
		...
		if( strncmp(sig, LOCAL_HDR_SIG, 4) != 0 )
