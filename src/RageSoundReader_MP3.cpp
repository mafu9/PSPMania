...
/*
 * PSP has a sub CPU that is called MediaEngine for distributing the load.
 * User threads haven't a permission to the CPU except only API.
 */

#define MP3_GET_TOTAL_SAMPLES	0x7FFFFFFF

#define MP3_DECODER_DELAY	529
#define SEEK_IGNORE_FRAMES	2

#define STR_ID3		0x00334449
#define STR_XING	0x676E6958
#define STR_INFO	0x6F666E49

static const uint16_t g_SampleRates[4][3] = {
...
static const uint16_t g_Bitrates[2][4][16] = {
	{	// MPEG1
		{ 0, },																			// reserved
...
static const uint16_t g_SamplePerFrame[4][4] = {
...
static int GetID3v2TagSize( uint8_t buf[10] )
{
	if( (*(uint32_t*)buf & 0x00FFFFFF) == STR_ID3 )
	{
		int size = buf[6];
		size = (size << 7) | buf[7];
		size = (size << 7) | buf[8];
...
int RageSoundReader_MP3::ReadMpegAudioHeader()
{
	uint8_t buf[4];
	...
	if( (header & 0xFFE00000) != 0xFFE00000			// invalid frame sync
		|| (header & 0x00600000) == 0x00000000		// invalid layer
		|| (header & 0x00180000) == 0x00080000		// invalid version
		|| (header & 0x0000F000) == 0x0000F000		// invalid bitrate
		|| (header & 0x0000F000) == 0x00000000		// invalid bitrate
		|| (header & 0x00000C00) == 0x00000C00 )	// invalid samplerate
	{
		//LOG->Trace( "invalid header %08X", header );
		file.SeekCur( -3 ); // seek back
		return -1;
	}

	version = (header & 0x00180000) >> 19;
	int layer = (header & 0x00060000) >> 17;
	int channel = (header & 0xC0) >> 6;
	int bitrateIndex = (header & 0xF000) >> 12;
	int sampleRateIndex = (header & 0xC00) >> 10;
	if( layer <= 0 || layer > 3
		|| version < 0 || version == 1 || version > 3
		|| channel < 0 || channel > 3
		|| bitrateIndex <= 0 || bitrateIndex >= 15
		|| sampleRateIndex < 0 || sampleRateIndex > 2 )
	...
	bitrate = g_Bitrates[version != 3 ? 1 : 0][layer][bitrateIndex];
	SampleRate = g_SampleRates[version][sampleRateIndex];
	Channels = (channel != 3 ? 2 : 1);
	samplePerFrame = g_SamplePerFrame[version][layer];
	int padding = (header & 0x200) >> 9;

	file.SeekCur( -4 ); // seek back

	return (125 * samplePerFrame * bitrate) / SampleRate + padding;
}

int RageSoundReader_MP3::GetEncoderDelayAndPadding()
{
	file.Seek( id3v2tagSize );

	int frameSize;
	while( (frameSize = ReadMpegAudioHeader()) < 0 )
	{
		SeekNextFrame();
		if( file.AtEOF() )
			return -1;
	...
	if( dataBuffer == NULL )
	{
		dataBuffer = (uint8_t*)memalign( 64, 1472 );
		if( dataBuffer == NULL )
			return -1;
	}

	if( file.Read( dataBuffer, frameSize ) != frameSize )
		return -1;
	...
	streamSize = file.GetFileSize() - id3v2tagSize;
	ASSERT( streamSize >= 0 );

	// vbrヘッダ
	uint32_t t = (p[3] << 24) | (p[2] << 16) | (p[1] << 8) | p[0];
	if( t == STR_XING || t == STR_INFO )
	{
		int offset = 8;
		if( p[7] & 0x01 ) // Frames field is present
		{
			// このフレームを含めないMP3フレームの合計数
			int frames = (p[offset] << 24) | (p[offset + 1] << 16) | (p[offset + 2] << 8) | p[offset + 3];
			totalSamples = frames * samplePerFrame;
			offset += 4;
		}

		if( p[7] & 0x02 ) // Bytes field is present
		{
			// このフレームを含めたMP3フレームの合計バイト数
			// 先頭のID3v2タグと末尾のID3v1タグのサイズは含まない
			int bytes = (p[offset] << 24) | (p[offset + 1] << 16) | (p[offset + 2] << 8) | p[offset + 3];
			streamSize = bytes - frameSize;
			offset += 4;
		}

		if( p[7] & 0x04 ) // TOC field is present
			offset += 100;

		if( p[7] & 0x08 ) // quality indicator field is present
		{
			//int quality = p[offset + 3];
			offset += 4;
		}

		p += offset + 21;
		encoderDelay = (p[0] << 4) | (p[1] >> 4);
		int encoderPadding = ((p[1] & 0x0F) << 8) | p[2];

		vbr = true;
		vbrHeaderSize = frameSize;
		totalSamples -= encoderDelay + encoderPadding;
	}

	file.Seek( id3v2tagSize + vbrHeaderSize );
	return 0;
}

RageSoundReader_MP3::RageSoundReader_MP3() : file( 8192 )
{
	mixBuffer = NULL;
	dataBuffer = NULL;
	codecBuffer = NULL;
...
RageSoundReader_MP3::~RageSoundReader_MP3()
{
	if( codecBuffer != NULL )
	{
		sceAudiocodecReleaseEDRAM( codecBuffer );
		free( codecBuffer );
	}
	free( dataBuffer );
	free( mixBuffer );
...
SoundReader_FileReader::OpenResult RageSoundReader_MP3::Open( const CString &filename_ )
{
	filename = filename_;
	...
	uint8_t tag[10];
	if( file.Read( tag, 10 ) != 10 )
		return OPEN_UNKNOWN_FILE_FORMAT;

	id3v2tagSize = GetID3v2TagSize( tag );

	if( GetEncoderDelayAndPadding() < 0 )
	{
		SetError( "GetEncoderDelayAndPadding failed" );
		return OPEN_UNKNOWN_FILE_FORMAT;
	}

	firstOffset = encoderDelay + MP3_DECODER_DELAY;
	return OPEN_OK;
}
...
SoundReader *RageSoundReader_MP3::Copy() const
{
	...
	ASSERT( b );
	ret->file.Seek( id3v2tagSize + vbrHeaderSize );

	ret->SampleRate = SampleRate;
	ret->Channels = Channels;
	ret->id3v2tagSize = id3v2tagSize;
	ret->streamSize = streamSize;
	ret->totalSamples = totalSamples;
	ret->firstOffset = encoderDelay + MP3_DECODER_DELAY;
	ret->vbrHeaderSize = vbrHeaderSize;
	ret->vbr = vbr;
	ret->encoderDelay = encoderDelay;
	ret->samplePerFrame = samplePerFrame;
	ret->bitrate = bitrate;
	ret->version = version;

	if( table.step >= 0 )
	{
		ret->table.fill = table.fill;
		ret->table.step = table.step;
		memcpy( ret->table.data, table.data, sizeof(table.data) );
	}
	else
	{
		ret->table.fill = 0;
...
int RageSoundReader_MP3::SeekNextFrame()
{
	uint8_t buf[512];
	...
	while( 1 )
	{
		...
		int size = file.Read( buf, sizeof(buf) );
		if( size <= 2 )
			return -1;
		...
		{
			uint16_t header = (buf[i] << 8) | buf[i + 1];
			if( (header & 0xFFE0) == 0xFFE0 )
				return file.Seek( offset + i );
		}
...
void RageSoundReader_MP3::UpdateSeekTable()
{
	if( table.step < 0 || frameNum != table.fill << table.step )
		return;
	
	if( table.fill == SEEK_TABLE_SIZE )
	{
		table.step++;
		table.fill >>= 1;
		for( int i = 0; i < table.fill; ++i )
			table.data[i] = table.data[i * 2];
	}

	if( frameNum == table.fill << table.step )
	{
		table.data[table.fill] = file.Tell();
		table.fill++;
	}
}

int RageSoundReader_MP3::DecodeFrame()
{
	int frameSize = ReadMpegAudioHeader();
	if( frameSize < 0 )
		return -1;
	...
	if( codecBuffer == NULL )
	{
		mixBuffer = (uint16_t*)memalign( 64, 1152 * 4 );
		codecBuffer = (unsigned long*)memalign( 64, 65 * 4 );
		if( dataBuffer == NULL )
			dataBuffer = (uint8_t*)memalign( 64, 1472 );

		if( mixBuffer == NULL || codecBuffer == NULL || dataBuffer == NULL )
		{
		...
		ret = sceAudiocodecCheckNeedMem( codecBuffer, PSP_CODEC_MP3 );
		if( ret < 0 )
		{
			SetError( ssprintf("Can't check audiocodec mem: %08X", ret) );
			free( codecBuffer );
			codecBuffer = NULL;
			return -1;
		}

		ret = sceAudiocodecGetEDRAM( codecBuffer, PSP_CODEC_MP3 );
		if( ret < 0 )
		{
			SetError( ssprintf("Can't get audiocodec EDRAM: %08X", ret) );
			free( codecBuffer );
			codecBuffer = NULL;
			return -1;
		}

		ret = sceAudiocodecInit( codecBuffer, PSP_CODEC_MP3 );
		if( ret < 0 )
		{
			SetError( ssprintf("Can't init audiocodec: %08X", ret) );
			return -1;
		}
	}
	...
	codecBuffer[6] = (unsigned long)dataBuffer;
	codecBuffer[8] = (unsigned long)mixBuffer;
	codecBuffer[9] = samplePerFrame * 4;
	codecBuffer[7] = codecBuffer[10] = frameSize;

	int ret = sceAudiocodecDecode( codecBuffer, PSP_CODEC_MP3 );
	if( ret < 0 )
	{
		// ファイルの先頭の無音ぽい部分にシークするとたまに0x807F00FDエラーが発生するが
		// どうせ無音だし問題にはならないだろうからゼロで埋めてそのまま再生する
		memset( mixBuffer, 0, 1152 * 4 );
		if( ret != (int)0x807F00FD )
		{
			SetError( ssprintf("sceAudiocodecDecode failed %08X %d %d", ret, file.Tell() - frameSize, frameNum) );
			return ret;
		}
	}
...
int RageSoundReader_MP3::Read( char *buf, unsigned len )
{
	...
	if( mixOutput < mixBufCount )
	{
		int copySamples = mixBufCount - mixOutput;
		if( copySamples + bufferFill > size )
			copySamples = size - bufferFill;
		
		memcpy( &data[bufferFill * 2], &mixBuffer[mixOutput * 2], copySamples * 4 );
		decodedSamples += copySamples;
		mixOutput += copySamples;
		bufferFill += copySamples;
	}

	while( bufferFill < size )
	{
		if( DecodeFrame() < 0 )
		{
			if( Error() )
		...
		mixBufCount = samplePerFrame;
		mixOutput = 0;

		// 末尾の要らないサンプルを出力バッファに入れないようにする
		if( vbr && decodedSamples + mixBufCount > totalSamples )
		{
			mixBufCount = totalSamples - decodedSamples;
			if( mixBufCount < 0 )
				mixBufCount = 0;
		}

		// 先頭の要らないサンプルを出力バッファに入れないようにする
		if( firstOffset > 0 )
		{
			if( mixOutput + firstOffset > mixBufCount )
			{
				firstOffset -= mixBufCount - mixOutput;
				mixOutput = mixBufCount;
			}
			else
			{
				mixOutput += firstOffset;
				firstOffset = 0;
			}
		}
...
int RageSoundReader_MP3::Seek( int sample )
{
	if( sample < 0 )
		sample = 0;

	if( sample == decodedSamples )
		return sample;

	int spf = samplePerFrame;
	int unadjustPos = sample + encoderDelay + MP3_DECODER_DELAY;
	int wantFrame = unadjustPos / spf;
	int ignoreFrame = wantFrame - SEEK_IGNORE_FRAMES;
	if( ignoreFrame < 0 )
		ignoreFrame = 0;

	if( table.step >= 0 && table.fill > 0 )
	{
		int index = ignoreFrame >> table.step;
		if( index >= table.fill )
			index = table.fill - 1;
		
		frameNum = index << table.step;
		file.Seek( table.data[index] );
	}

	int decoded = sample + (frameNum - ignoreFrame) * spf;
	int tsMinusSpf = (totalSamples > 0 ? totalSamples - spf : 0x7FFFFFFF);
	
	while( frameNum < ignoreFrame )
	{
		int frameSize = ReadMpegAudioHeader();
		if( frameSize < 0 )
		{
		...
		file.SeekCur( frameSize );

		if( decoded > tsMinusSpf )
		{
			decoded = totalSamples;
			break;
		}

		decoded += samplePerFrame;
	}

	decodedSamples = decoded;
	firstOffset = unadjustPos - ignoreFrame * spf;
	mixOutput = 0;
	mixBufCount = 0;
	return decoded;
}
...
{
	int sample = (int)((int64_t)ms * (int64_t)SampleRate / 1000LL);
	return (int)((int64_t)Seek( sample ) * 1000LL / (int64_t)SampleRate);
}
...
int RageSoundReader_MP3::SetPosition_Fast( int ms )
{
	...
	file.Seek( id3v2tagSize + ignoreFrame * frameSize );
	while( ReadMpegAudioHeader() < 0 )
	{
		SeekNextFrame();
		if( file.AtEOF() )
		{
...
{
	if( totalSamples > 0 )
		return (int)((int64_t)totalSamples * 1000LL / (int64_t)SampleRate);

	if( fast && !vbr )
		return file.GetFileSize() * 8 / bitrate;

	if( totalSamples <= 0 )
	{
		int origDecodedSamples = decodedSamples;
		int origFrameNum = frameNum;
		int origFirstOffset = firstOffset;
		int origFilePos = file.Tell();

		if( Seek( MP3_GET_TOTAL_SAMPLES ) < 0 )
			return -1;

		totalSamples = decodedSamples;
		decodedSamples = origDecodedSamples;
		frameNum = origFrameNum;
		firstOffset = origFirstOffset;
